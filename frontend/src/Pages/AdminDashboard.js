...
function AdminDashboard() {
  ...
  const [services, setServices] = useState([{ description: "", qty: "1", unit: "", price: "", unitPrice: "" }]);
  ...
  const updateService = (index, field, value) => {
    setServices(prev => {
      ...
      if (field === "qty") {
        ...
        const unit = parseFloat(currentRow.unitPrice) || 0;
  ...
  const updatePart = (index, field, value) => {
    setParts(prev => {
      ...
      // AUTO-FILL UNIT PRICE WHEN CODE IS TYPED
      if (field === "description") {
        ...
        if (product) {
          copy[index].unitPrice = product.srp || product.price; // store base price
          copy[index].price = product.srp || product.price;
        }
      }

      // INCREASE PRICE WHEN QTY CHANGES
      if (field === "qty") {
        const qty = parseFloat(value) || 0;
        const unit = parseFloat(copy[index].unitPrice || copy[index].price || 0);

        copy[index].price = (qty * unit).toFixed(2);
      }
  ...
  const resetJobForm = () => {
    ...
    setStatus("Pending");
    setAddress("");
    setServices([{ description: "", qty: "1", unit: "", price: "", unitPrice: "" }]);
  ...
  const [newProduct, setNewProduct] = useState({ name: "", quantity: "", price: "", srp: "", companyCodename: "" });
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const result = await createInventoryItem({
        ...
        srp: parseFloat(newProduct.srp),
        companyCodename: newProduct.companyCodename
      });

      if (result.success) {
        await loadData();
        setNewProduct({ name: "", quantity: "", price: "", srp: "" });
        setShowModal(false);
      } else {
  ...
  const openEditModal = (productId) => {
    ...
    setIsEditModal(true);
    setNewProduct({ code: p.code, name: p.name, quantity: p.stocks, price: p.price, srp: p.srp, companyCodename: p.company_codename });
  };
...
pdfMake.createPdf(docDefinition)
  .download(`JOB-ORDER-${formatJobOrderNo(jobData.joNumber)}.pdf`);
  };
        ...
        {activeTab === "inventory" && (
          <>
            <div className="inventory-header">
              ...
              <button className="add-product-btn" onClick={() => { setNewProduct({ name: "", quantity: "", price: "", srp: "" }); setShowModal(true); }}>Add Product</button>
            </div>
            ...
            <thead>
              ...
              <tbody>
                {products.length === 0 ? (
                  <tr><td colSpan="8" className="empty-message">No products added yet.</td></tr>
                ) : (
                  products.map((p) => (
                    <tr key={p.id}>
                      <td>{p.code}</td>
                      ...
                      <td>₱{parseFloat(p.srp).toFixed(2)}</td>
                      <td>
        ...
        {showModal && (
          <div className="modal-container">
            <div className="modal-box">
              ...
              <label>SRP</label>
              <input type="number" placeholder="SRP" step="0.01" value={newProduct.srp} onChange={(e) => setNewProduct({ ...newProduct, srp: e.target.value })} />
        ...
        {isEditModal && (
          <div className="modal-container">
            <div className="modal-box">
              ...
              <label>SRP</label>
              <input type="number" step="0.01" value={newProduct.srp} onChange={(e) => setNewProduct({ ...newProduct, srp: e.target.value })} />
              <div className="modal-actions">
